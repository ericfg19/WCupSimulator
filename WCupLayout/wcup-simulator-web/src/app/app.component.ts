...
export class AppComponent implements OnInit {
  ...
  changeQuarter(lastMatch: number, winner: Team) {
    ...
    if (isTeamOneSide) this.quarter[currentMatch].teamOne = winner;
    else this.quarter[currentMatch].teamTwo = winner;

    var nextMatch = this.calculateNextMatch(currentMatch);
    var loser =
      this.roundOf16[lastMatch].teamOne!.name == winner.name
        ? this.roundOf16[lastMatch].teamTwo
        : this.roundOf16[lastMatch].teamOne;

    if (
      this.semi[nextMatch].teamOne == loser ||
      this.semi[nextMatch].teamTwo == loser
    )
  ...
  changeSemi(
    ...
    if (
      this.quarter[lastMatch].teamOne?.name == '' ||
    ...
    if (isTeamOneSide) this.semi[currentMatch].teamOne = winner;
    else this.semi[currentMatch].teamTwo = winner;

    if (loser == undefined)
      loser =
        this.quarter[lastMatch].teamOne!.name == winner.name
          ? this.quarter[lastMatch].teamTwo
          : this.quarter[lastMatch].teamOne;
  ...
  changeFinal(
    ...
    if (
      this.semi[lastMatch].teamOne?.name == '' ||
    ...
    if (loser == undefined)
      loser =
        this.semi[lastMatch].teamOne!.name == winner.name
          ? this.semi[lastMatch].teamTwo
          : this.semi[lastMatch].teamOne;

    if (this.winner == loser) this.changeWinner(winner);
  }

  changeWinner(winner: Team) {
    if (this.final.teamOne?.name == '' || this.final.teamTwo?.name == '')
      return;
    this.winner = winner;
  }
